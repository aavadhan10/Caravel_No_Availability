...
def detect_csv_structure(df):
    ...
    # Detect name columns - be more specific
    name_patterns = ['name', 'submitter name', 'first name', 'last name', 'lawyer name', 'attorney', 'counsel']
    for i, col in enumerate(columns_lower):
        if any(pattern == col or col.endswith(' name') for pattern in name_patterns):
            structure['name_columns'].append(df.columns[i])
    
    # Detect email columns
    email_patterns = ['email', 'e-mail', 'mail', 'contact email']
    for i, col in enumerate(columns_lower):
        if any(pattern in col for pattern in email_patterns):
            structure['email_columns'].append(df.columns[i])
    
    # Detect bio.blurb columns specifically - be more precise
    blurb_patterns = ['blurb', 'bio.blurb', 'biography', 'bio', 'profile', 'summary', 'description']
    for i, col in enumerate(columns_lower):
        if any(pattern == col for pattern in blurb_patterns):
            structure['blurb_columns'].append(df.columns[i])
    
    # Detect skill columns (columns with numeric values or skill-like names)
    skill_patterns = ['skill', 'expertise', 'competency', 'rating', 'score']
    for i, col in enumerate(columns_lower):
        ...
        # Or if it contains mostly numeric values and isn't already categorized
        elif (df[df.columns[i]].dtype in ['int64', 'float64'] and 
              df.columns[i] not in structure['blurb_columns'] and
              df.columns[i] not in structure['name_columns']):
            structure['skill_columns'].append(df.columns[i])
    ...
    bio_patterns = ['level', 'title', 'jurisdiction', 'location', 'education', 'firm', 'company', 
                   ...
                   'previous', 'notable', 'expert', 'background', 'qualification', 'practice area']
    for i, col in enumerate(columns_lower):
        if (any(pattern in col for pattern in bio_patterns) and
            df.columns[i] not in structure['blurb_columns'] and
            df.columns[i] not in structure['name_columns'] and
            df.columns[i] not in structure['email_columns']):
...
def extract_lawyer_info_flexible(row, structure):
    ...
    """
    # Extract name - be more careful about what we consider a "name"
    name = ""
    if structure['name_columns']:
        ...
        for col in structure['name_columns']:
            if pd.notna(row[col]) and str(row[col]).strip():
                ...
                # Filter out obviously non-name entries
                if (len(candidate_name) < 100 and  # Names shouldn't be super long
                    not candidate_name.lower().startswith('maybe helped') and
                    not 'one son' in candidate_name.lower() and
                    not 'two dogs' in candidate_name.lower() and
                    not candidate_name.lower().startswith('http') and
                    '.' not in candidate_name[:20]):  # Avoid URLs or sentences
                    name_parts.append(candidate_name)
        name = " ".join(name_parts)
    
    # If no good name found, try first text column but with similar filters
    if not name and structure['text_columns']:
        for col in structure['text_columns'][:3]:
            if pd.notna(row[col]) and str(row[col]).strip():
                candidate_name = str(row[col]).strip()
                if (len(candidate_name) < 50 and 
                    not candidate_name.lower().startswith('maybe') and
                    not 'son' in candidate_name.lower() and
                    not 'dog' in candidate_name.lower() and
                    ' ' in candidate_name and  # Should have first and last name
                    len(candidate_name.split()) <= 4):  # Not too many words
                    name = candidate_name
                    break
    
    # Fallback name if still nothing good
    if not name:
        name = f"Lawyer {row.name if hasattr(row, 'name') else 'Unknown'}"
    
    # Extract email
    email = ""
    if structure['email_columns']:
        for col in structure['email_columns']:
            if pd.notna(row[col]) and str(row[col]).strip() and '@' in str(row[col]):
                email = str(row[col]).strip()
                break
    
    # If no email found, generate a cleaner placeholder
    if not email:
        clean_name = re.sub(r'[^a-zA-Z\s]', '', name.lower())
        name_parts = clean_name.split()[:2]  # First two words only
        email_name = '.'.join(name_parts) if len(name_parts) > 1 else name_parts[0] if name_parts else 'lawyer'
        email = f"{email_name}@example.com"
    
    # Extract bio.blurb with highest priority - be more specific about what's a blurb
    blurb = ""
    if structure['blurb_columns']:
        for col in structure['blurb_columns']:
            if pd.notna(row[col]) and str(row[col]).strip():
                candidate_blurb = str(row[col]).strip()
                # Blurbs should be substantial but not super long sentences
                if (len(candidate_blurb) > 50 and len(candidate_blurb) < 2000 and
                    not candidate_blurb.lower().startswith('maybe') and
                    not 'one son' in candidate_blurb.lower()):
                    blurb = candidate_blurb
                    break
    
    # Extract skills - be more selective
    skills = {}
    for col in structure['skill_columns']:
        if pd.notna(row[col]):
            ...
            try:
                value = float(value)
                if value > 0 and value <= 10:  # Reasonable skill score range
...
if st.session_state.get('show_admin_dashboard', False) and st.session_state.get('admin_mode', False):
                ...
                for match in sorted_matches:
                    ...
                    with st.container():
                        ...
                        with match_col2:
                            ...
                            # Supporting skills - CLEAN FORMAT
                            if matched_skills:
                                st.markdown("**🔧 Supporting Skills:**")
                                # Show only top 3 skills in a clean format
                                for i, skill in enumerate(matched_skills[:3]):
                                    st.markdown(f"- {skill['skill']}: {skill['value']}")
                                if len(matched_skills) > 3:
                                    st.markdown(f"- *...and {len(matched_skills) - 3} more*")
                        ...
                        # Reasoning section - BETTER FORMATTING
                        st.markdown("**🎯 WHY THIS LAWYER IS AN EXCELLENT MATCH:**")
                        
                        # Put reasoning in a nice container
                        with st.container():
                        ...
                        st.markdown("")
                        st.markdown("---")
                        st.markdown("")  # Extra space between lawyers
                
                # Action buttons for results
                st.markdown("### Actions")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("📧 Email These Matches to Requester", key=f"email_matches_{len(matches)}", use_container_width=True):
                        st.success("✅ Match results have been emailed to the requester!")
                        st.balloons()
                with col2:
                    if st.button("📆 Schedule Consultation", key=f"schedule_consult_{len(matches)}", use_container_width=True):
    ...
    st.markdown(
        ...
        "Last updated: July 8, 2025"
                    ...
                    if len(skill_name) > 0 and len(skill_name) < 50:  # Reasonable skill name length
                        skills[skill_name] = value
            except (ValueError, TypeError):
                # If it's text, be more selective
                if str(value).strip() and len(str(value).strip()) < 50:
                    skill_name = col.strip()
                    skills[skill_name] = 1.0
    
    # Extract biographical information with better filtering
    bio = {}
    bio_field_mapping = {
        ...
        'blurb': ['blurb', 'bio.blurb', 'biography', 'summary', 'description', 'profile']
    }
    ...
    # Map columns to bio fields with better filtering
    for col in structure['bio_columns'] + structure['text_columns'] + structure['blurb_columns']:
        col_lower = col.lower()
        value = str(row[col]) if pd.notna(row[col]) else ""
        
        if value.strip() and len(value.strip()) < 500:  # Filter out extremely long entries
            # Skip obviously bad data
            if (value.lower().startswith('maybe helped') or 
                'one son' in value.lower() or 
                'two dogs' in value.lower()):
                continue
            ...
            # If no specific match, add to notable_items or expert (but be selective)
            if not best_match and len(value) < 200:  # Don't add super long text
